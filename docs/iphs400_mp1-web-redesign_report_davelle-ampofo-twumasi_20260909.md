...
**Type system.** The site ran on Fraunces, Newsreader, and Quicksand. Quicksand is a rounded geometric sans and it was making an upper-level CS course read like a children's product. I replaced the stack with Instrument Serif for display, Inter for reading and interface text, and JetBrains Mono for labels, dates, and numerals. The mono labels do most of the work: they give the page a technical register that suits a course about agentic software engineering. Instrument Serif ships a single weight, so heading hierarchy comes from size and spacing rather than bold, which forced a cleaner scale than I would have written otherwise.

**Course arc.** The homepage had an SVG of fifteen coloured dots along a wavy path. It looked like a board game. I replaced it with a proportional phase band over an indexed list of all fifteen weeks, with the current week marked. The index is now real navigation rather than decoration sitting next to it.
...
**Today in AI.** The existing widget ran one fixed keyword list against stories posted that day, so all 22 pages showed identical headlines. Each week now has its own search terms, so the headlines beside Week 8 are about Codex CLI and the ones beside Week 12 are about LLM costs.

**Accessibility.** I measured the palette and found the light theme was below WCAG AA. The teal accent sat at 3.15:1 against the background and the muted grey at 3.51:1, against a 4.5:1 floor. That covered every body link and, after I shrank the labels during the type pass, nearly every label on the site. I darkened both while holding hue and saturation, solving against the tinted background of the highlighted row rather than the plain one, since that is the worst case. Dark mode was already past 7:1.
...
- *"i dont want icons, that is ai slop"* — killed a proposed icon system before it was built.
- *"spec it out let me see"* — required a written spec before the Schedule rebuild. The spec surfaced a decision I would otherwise have made badly: whether to generate the page from JavaScript or keep it static. Static won, because the repo's own tests parse HTML.
