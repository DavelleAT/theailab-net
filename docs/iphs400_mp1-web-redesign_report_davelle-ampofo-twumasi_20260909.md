...
**Today in AI.** This widget is mine, not something the starting site had. The idea was that a course about a field moving this fast should show the field moving, so the page pulls live AI headlines from Hacker News beside the week you are reading.

It also produced the most instructive failure of the project. My first version ran a fixed keyword list against stories posted that day, which was thin. My second gave each week its own search terms, so the card beside Week 8 would search "Codex CLI" and the one beside Week 12 would search "LLM cost". On paper that is clearly better. In practice it was worse, and it took looking at the actual output to see why: those phrases barely occur in news articles. Where they occur on Hacker News is in "Show HN" posts, so searching for them was really searching for other people's side projects, and the card filled with links to GitHub repos.

The fix was to stop searching. It now pulls recent AI stories sorted by how much the site discussed them, drops anything titled Show HN, Launch HN, Ask HN or Tell HN, drops links that go straight to a code host, and applies a points floor. The result is frontier model news: GPT-6 Astra, Claude Fable 5.1, Gemini 3.8 Flash. The lesson I would keep is that a feature can be well built, well tested, and still wrong, and no amount of refining the clever version gets you to the plain version that works.
...
- The most useful habit was refusing to accept plausible output. The fifteen per-week search topics for the news widget looked fine written down. Measured against the live API, six returned nothing at all, so I rewrote and re-measured all fifteen. Then I read the headlines the surviving ones produced and found they were nearly all Show HN posts, which is what finally killed the whole approach. Measuring the wording caught a bad list. Reading the output caught a bad idea. The second is the one that mattered, and it is the step easiest to skip once the numbers look healthy.
